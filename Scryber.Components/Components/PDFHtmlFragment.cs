...
namespace Scryber.Components
{
    ...
    public class PDFHtmlFragment : Scryber.Components.PDFVisualComponent
    {
        ...
        public PDFHtmlFragment()
            : this((PDFObjectType)"htmF")
        {
        }

        protected PDFHtmlFragment(PDFObjectType type)
            : base(type)
        {
        }
        ...
        protected virtual void EnsureContentsParsed(PDFContextBase context, bool performload)
        {
            ...
            try
            {
                if (_added != null && _added.Count > 0)
                {
                    ...
                    {
                        container.Content.Remove(prev as PDFComponent);
                    }
                ...
                if (_added.Count > 0)
                {
                    ...
                    if (performload)
                    {
                        ...
                        for (int i = 0; i < _added.Count; i++)
                        {
                            ...
                            if (comp is PDFVisualComponent)
                                (comp as PDFVisualComponent).Load(loadContext);
                        }
                    }
                }
            }
