...
namespace Scryber.Components
{
    ...
    public class PDFLink : PDFVisualComponent, IPDFInvisibleContainer
    {
        ...
        private void FillActionAnnotations(PDFLayoutContext context, Styles.PDFStyle style,
                                           ...
                                           Layout.PDFLayoutPage pg, PDFComponentList contents)
        {
            foreach (PDFComponent comp in contents)
            {
                PDFAnnotationLinkEntry annot;
                PDFComponentList inner;
        ...
        protected bool IsContainer(PDFComponent comp, out PDFComponentList list)
        {
            if (comp is PDFPanel)
            {
                PDFPanel panel = comp as PDFPanel;
                if (panel.HasContent)
                {
                    list = panel.Contents;
                    return true;
