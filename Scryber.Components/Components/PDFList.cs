...
namespace Scryber.Components
{
    ...
    public class PDFList : PDFPanel
    {

        #region public PDFListItemList Items {get;}

        private PDFListItemList _items;

        /// <summary>
        /// Gets the collection of PDFListItem(s). This will return a wrapped collection of List Items 
        /// </summary>
        [PDFArray(typeof(PDFListItem))]
        [PDFElement("")]
        public PDFListItemList Items
        {
            get
            {
                if (this._items == null)
                    this._items = new PDFListItemList(this.InnerContent);
                return this._items;
            }
        }
        ...
        protected PDFList(PDFObjectType type)
            : base(type)
        {
