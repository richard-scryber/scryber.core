...
namespace Scryber.Data
{
    ...
    public class PDFDataGrid : Scryber.Components.PDFDataTemplateComponent
    {
        //
        // properties
        //
        ...
        /// </summary>
        [PDFElement("Columns")]
        [PDFArray(typeof(PDFDataGridColumn))]
        ...
        /// </summary>
        private PDFTableGrid _built = null;
        ...
        protected PDFDataGrid(PDFObjectType type)
            : base(type)
        {
        }
        ...
        protected override void DoBindDataIntoContainer(IPDFContainerComponent container, int containerposition, PDFDataContext context)
        {
            ...
            PDFTableGrid grid = new PDFTableGrid();
            ...
            {
                PDFTableHeaderRow header = new PDFTableHeaderRow();
                ...
                foreach (PDFDataGridColumn column in this.Columns)
                {
                    ...
                    PDFComponent comp = column.DoBuildHeaderCell(grid, header, rowindex, colindex, context);
            ...
            while (enumerator.MoveNext())
            {
                ...
                foreach (PDFDataGridColumn column in this.Columns)
                {
                    ...
                    PDFComponent comp = column.DoBuildItemCell(grid, row, rowindex, colindex, context);
                    if (null != comp)
            ...
            {
                PDFTableFooterRow footer = new PDFTableFooterRow();
                ...
                foreach (PDFDataGridColumn column in this.Columns)
                {
                    ...
                    PDFComponent comp = column.DoBuildFooterCell(grid, footer, rowindex, colindex, context);
                    if (null != comp && null != parentData)
        ...
        private bool RemoveHiddenColumns(PDFDataContext context)
        {
            ...
            {
                PDFDataGridColumn column = this.Columns[i];
        ...
        private void RemoveHeadersAndFooters(PDFTableGrid grid, PDFDataContext context)
        {
        ...
        protected virtual void AddAutoBindColumns(PDFDataContext context)
        {
            ...
            else
            {
                foreach (PDFDataItem item in schema.Items)
                {
                    ...
                    {
                        PDFDataGridColumn col = GetDataColumnForType(item.DataType);
                        if (null != col)
                        {
        ...
        private PDFDataGridColumn GetDataColumnForType(DataType type)
        {
            PDFDataGridColumn col = null;
            switch (type)
            {
                ...
                case DataType.Unknown:
                    col = new PDFDataGridTextColumn();
                    break;
                case DataType.Html:
                    col = new PDFDataGridHtmlColumn();
                    break;
                case DataType.Integer:
                case DataType.Double:
                    col = new PDFDataGridNumberColumn();
                    break;
                case DataType.DateTime:
                    col = new PDFDataGridDateColumn();
                    break;
                case DataType.Image:
                    col = new PDFDataGridImageDataColumn();
                    break;
                case DataType.Url:
                    col = new PDFDataGridLinkColumn();
                    break;
                default:
                    break;
            }
        ...
        private void InitAndLoadRoot(PDFComponent built, PDFContextBase context)
        {
            if(context.TraceLog.ShouldLog(TraceLevel.Debug))
            ...
            PDFInitContext init = new PDFInitContext(context.Items, log, context.PerformanceMonitor);
            PDFLoadContext load = new PDFLoadContext(context.Items, log, context.PerformanceMonitor);
