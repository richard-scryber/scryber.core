...
namespace Scryber.Data
{
    ...
    public abstract class PDFDataGridColumn : PDFVisualComponent
    {
        ...
        PDFTableGrid gridRef = null;

        //
        // .ctor
        //

        protected PDFDataGridColumn(PDFObjectType type)
            : base(type)
        {
        }
        ...
        public virtual PDFComponent DoBuildHeaderCell(PDFTableGrid grid, PDFTableRow row, int rowindex, int columnindex, PDFDataContext context)
        {
        ...
        {
            PDFTableCell cell = (PDFTableCell)sender;

            ApplyHeaderCellStyles(cell);
        }

        private void ApplyHeaderCellStyles(PDFTableCell cell)
        {
        ...
        public virtual PDFComponent DoBuildItemCell(PDFTableGrid grid, PDFTableRow row, int rowindex, int columnindex, PDFDataContext context)
        {
            if (context.TraceLog.ShouldLog(TraceLevel.Debug))
        ...
        public virtual PDFComponent DoBuildFooterCell(PDFTableGrid grid, PDFTableRow row, int rowindex, int columnindex, PDFDataContext context)
        {
            if (context.TraceLog.ShouldLog(TraceLevel.Debug))
            ...
            PDFTableFooterCell cell = new PDFTableFooterCell();
            row.Cells.Add(cell);
        ...
        private void HideEntireColumnAsAllInivisible()
        {
            if (gridColumnIndex > -1 && gridRef != null)
            {
                ...
                foreach (PDFTableRow row in gridRef.Rows)
                {
                    PDFTableCell cell = row.Cells[gridColumnIndex];
    ...
    public class PDFDataGridColumnCollection : PDFComponentWrappingList<PDFDataGridColumn>
    {

        public PDFDataGridColumnCollection(PDFDataGrid grid)
            : base(new PDFComponentList(grid,(PDFObjectType)"DgCo"))
        {
        }
    }
}
