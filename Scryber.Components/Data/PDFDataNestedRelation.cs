...
namespace Scryber.Data
{
    ...
    public class PDFDataNestedRelation : PDFDataRelation
    {
        
        public PDFDataNestedRelation()
            : this(PDFObjectTypes.SqlNestedRelationType)
        {
        }

        protected PDFDataNestedRelation(PDFObjectType type)
            : base(type)
        {
        }
        ...
        public override void AddRelation(IPDFDataSetProviderCommand parentCommand, IPDFDataSetProviderCommand childCommand, System.Data.DataSet dataset, PDFDataContext context)
        {
            ...
            if (null == child)
            {
                context.TraceLog.Add(TraceLevel.Message, PDFSqlProviderCommand.SqlCommandLog, "Relation not created between '" + childtablename + "' and '" + parenttablename + "' as one or more tables are not in data set");
                return;
            }
            ...
            {
                PDFDataRelationMatch match = this.MatchOn[i];
            ...
            DataRelation rel = new DataRelation(childtablename + "_2_" + parenttablename, parentmatches, childmatches);
            rel.Nested = true;
