...
namespace Scryber.Data
{
    ...
    public class PDFObjectParameter : IPDFBindableComponent
    {
        //
        ...
        /// <returns></returns>
        public object GetNativeValue(PDFObjectProviderCommand command, System.Type paramtype)
        {
    ...
    /// </summary>
    public class PDFObjectParameterList : List<PDFObjectParameter>
    {

        public PDFObjectParameter this[string name]
        {
            get
            {
                for (int i = 0; i < this.Count; i++)
                {
                    PDFObjectParameter p = this[i];
        ...
        public void DataBind(PDFDataContext context)
        {
            if (this.Count > 0)
            {
                foreach (PDFObjectParameter param in this)
                {
