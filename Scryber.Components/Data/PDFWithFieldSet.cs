...
namespace Scryber.Data
{
    ...
    public class PDFWithFieldSet : PDFWith
    {
        ...
        private PDFWithFieldCollection _fields;

        /// <summary>
        /// Gets the collection of fields in this set
        /// </summary>
        [PDFElement("Fields")]
        [PDFArray(typeof(PDFWithField))]
        public PDFWithFieldCollection Fields
        {
            get
            {
                if (null == this._fields)
                    this._fields = new PDFWithFieldCollection(this.InnerContent);
                return _fields;
            }
        }
        ...
        public PDFWithFieldSet() : this((PDFObjectType)"WtFs")
        {
        }

        protected PDFWithFieldSet(PDFObjectType type) 
            : base(type)
        {
        }
        ...
        protected override void DoBindDataIntoContainer(IPDFContainerComponent container, int containerposition, PDFDataContext context)
        {
            ...
            foreach(PDFWithField fld in this.Fields)
            {
        ...
        protected virtual void AddAutoBindFields(object data, IPDFDataSource source, PDFDataContext context)
        {
            ...
            else
            {
                ...
                foreach (PDFDataItem item in schema.Items)
                {
                    ...
                    {
                        PDFWithBoundField field = GetFieldForType(item.DataType);
                        if (null != field)
                        {
        ...
        protected virtual PDFWithBoundField GetFieldForType(Scryber.DataType type)
        {
            PDFWithBoundField fld;
            switch (type)
            {
                case DataType.Integer:
                case DataType.Double:
                    fld = new PDFWithNumberField();
                    break;

                case DataType.DateTime:
                    fld = new PDFWithDateField();
                    break;

                case DataType.Boolean:
                    fld = new PDFWithBooleanField();
                    break;
                ...
                case DataType.Unknown:
                    fld = new PDFWithTextField();
                    break;

                case DataType.Html:
                    fld = new PDFWithHtmlField();
                    break;

                case DataType.Image:
                    fld = new PDFWithImageField();
                    break;
