...
namespace Scryber
{
    ...
    public class RemoteFileAsyncRequestSet : RemoteFileRequestSet
    {
        ...
        {
            if (this.LogVerbose)
                this.AddVerboseLog("Beginning wait for remote request completion to path " + request.FilePath);
            ...
            if (!request.IsCompleted)
            {
                ...
                if (raiseErrors)
                    throw new InvalidOperationException("The remote request for " + request.FilePath + " timed out. Extend the maximum duration of " + (this.WaitDurationMillisecond / 1000.0) + "s for the remote request");
                else
                    this.Log.Add(TraceLevel.Error, RemoteRequestCategory, "Could not complete the remote request for " + request.FilePath + " within the available time of " + (this.WaitDurationMillisecond / 1000.0) + "s. Extend the maximum duration for the remote request to appease this error");
                return false;
            }
            else
            {
                if (LogVerbose)
                    this.AddVerboseLog("Completed the request successfully for " + request.FilePath + " after " + (count / 1000.0) + "s");
