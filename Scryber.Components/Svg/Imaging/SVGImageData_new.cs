...
namespace Scryber.Svg.Imaging
{
    ...
    public class SVGPDFImageData_new : ImageVectorData, ILayoutComponent
    {
        ...
        public Size GetRequiredSizeForLayout(Size available, LayoutContext context, Style appliedstyle)
        {
            ...
            if (canvasStyle.TryGetValue(StyleKeys.SizeWidthKey, out var width))
            {
                newSize.Width = width.Value(canvasStyle);
                if (canvasStyle.TryGetValue(StyleKeys.SizeHeightKey, out var height))
                {
        ...
        protected virtual PDFLayoutBlock DoLayoutCanvas(LayoutContext context, Style style)
        {
            ...
            try
            {
                ...
                //used in the calculation of relative sizes.
                ...
                if (applied.IsValueDefined(StyleKeys.SizeWidthKey) && false)
                {
                    ...
                    if (width.IsRelative) //we have a width and it is proportional
                    {
                        switch (width.Units)
                        {
                            case (PageUnits.Percent):
                                width = width.ToAbsolute(open.AvailableBounds.Width);
                                break;
                            case (PageUnits.EMHeight):
                                width = width.ToAbsolute(txtSize.Height);
                                break;
                            case (PageUnits.EXHeight):
                                width = width.ToAbsolute(txtSize.Height); //same as zero-width
                                break;
                            case (PageUnits.RootEMHeight):
                                width = width.ToAbsolute(defSize);
                                break;
                            case (PageUnits.ViewPortHeight):
                                width = width.ToAbsolute(open.AvailableBounds.Height);
                                break;
                            case (PageUnits.ViewPortWidth):
                                width = width.ToAbsolute(open.AvailableBounds.Width);
                                break;
                            case (PageUnits.ViewPortMax):
                                var max = Unit.Max(open.AvailableBounds.Width, open.AvailableBounds.Height);
                                width = width.ToAbsolute(max);
                                break;
                            case (PageUnits.ViewPortMin):
                                var min = Unit.Min(open.AvailableBounds.Width, open.AvailableBounds.Height);
                                width = width.ToAbsolute(min);
                                break;
                            default:
                                break;
                        }
                        ...
                        {
                            height = applied.GetValue(StyleKeys.SizeHeightKey, Unit.Auto);
                        }
                        else if (viewbox != Rect.Empty) // we have a view box - so go proportional
                        {
                        }
                    }
                    ...
                    {
                        StyleKeys.SizeWidthKey.FlattenValue(applied, pg.Size, open.AvailableBounds.Size,
                            txtSize, defSize);
                        width = applied.GetValue(StyleKeys.SizeWidthKey, Unit.Auto);
                    }
                }
                ...
                else if (height.IsRelative && false)
                {
                    ...
                    {
                        height = height.ToAbsolute(open.AvailableBounds.Width);
                    }
                    else
                    {
                        StyleKeys.SizeHeightKey.FlattenValue(applied, pg.Size, open.AvailableBounds.Size,
                            txtSize, defSize);
                        height = applied.GetValue(StyleKeys.SizeHeightKey, Unit.Auto);
                    }
                }
                ...
                var full = layoutContext.StyleStack.GetFullStyle(this._svgCanvas,
                    ...
                    (IComponent forcomponent, Style withstyle, PositionMode withposition) => { return new Size(width, height);}, txtSize, defSize);
