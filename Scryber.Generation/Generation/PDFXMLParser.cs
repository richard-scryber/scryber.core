...
namespace Scryber.Generation
{
    public class PDFXMLParser : IPDFParser
    {
        ...
        /// <returns></returns>
        /// <returns></returns>
        public IPDFComponent Parse(string source, XmlReader reader, ParseSourceType type)
        {
            try
            {
            ...
            while (reader.Read())
            {
                ...
                if (reader.NodeType == XmlNodeType.Element)
                {
                    ...
                    if (!(value is IPDFComponent))
                        throw BuildParserXMLException(reader, Errors.CannotConvertObjectToType, value.GetType(), typeof(IPDFComponent));
        ...
        protected virtual void EnsureAllOutletsAreAssigned(XmlReader reader, IPDFComponent parsed)
        {
            if (parsed is IPDFControlledComponent && this.HasController)
            {
                ...
                ((IPDFControlledComponent)parsed).Controller = this.Controller;
            }
        }
        ...
        /// <param name="parsed"></param>
        protected virtual void ApplyParsedComponentProperties(string source, ParserLoadType loadtype, IPDFComponent parsed)
        {
            ...
            if (parsed is IPDFXMLParsedDocument)
            {
