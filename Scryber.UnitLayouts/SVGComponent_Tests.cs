...
namespace Scryber.UnitLayouts
{
    ...
    public class SVGComponent_Tests
    {
        ...
        {
            var path = AssertGetContentFile("SVGComponents_32_Use_Ellipse");

            var doc = Document.ParseDocument(path);

            using (var ms = DocStreams.GetOutputStream("SVGComponents_32_Use_Ellipse.pdf"))
            {
                doc.Pages[0].Style.OverlayGrid.ShowGrid = true;
                doc.Pages[0].Style.OverlayGrid.GridSpacing = 10;
                doc.Pages[0].Style.OverlayGrid.GridColor = StandardColors.Aqua;
            ...
            var expectedBounds = new Rect();
            //cx = 20, cy = 30, r = 20;
            expectedBounds.X = svgBounds.X + 45 - 40;
            expectedBounds.Y += svgBounds.Y + 30 - 20;
