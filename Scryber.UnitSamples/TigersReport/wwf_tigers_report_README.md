...
- **JSON-Powered**: All content in an editable JSON file
...
2. **wwf_tigers_report_data.json** - JSON data file with all report content
3. **wwf_tigers_report_model.cs** - C# code to load JSON and generate PDF
4. **wwf_tigers_report_README.md** - This documentation file
...
### 6. CSS Styling
- Comprehensive inline CSS for professional report layout
...
Change these values to match your organization's brand colors.

### Sections

The template is modular. You can:
...
- Modify section styling independently

### Data Structure

The data model is flexible:
- Optional properties use `{{#if}}` checks
...
**Template Version**: 2.1 (Simplified JSON binding - Scryber handles everything automatically)
